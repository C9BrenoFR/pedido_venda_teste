...
async function loadOrderDetails(status = currentFilters.status) {
    ...
    try {
        ...
        const repCnpj = clienteCNPJFilter ? `&cnpj=${clienteCNPJFilter}`:'';
...
document.getElementById('applyFilters').addEventListener('click', async () => {
    await loadOrderDetails(status = currentFilters.status);
});
...
async function applyFilters() {
   ...
   showFeedback("Buscando dados, aguarde...");
    ...
    try {
        ...
        // Formatar as datas antes de enviá-las
        const dataInicio = formatDate(currentFilters.dataInicio);
        const dataFim = formatDate(currentFilters.dataFim);

        // Atualiza a URL com os filtros atuais
        let queryParams = `status=${currentFilters.status}`;
        queryParams += currentFilters.representante ? `&codRep=${currentFilters.representante}` : '';
        queryParams += currentFilters.clienteCNPJ ? `&cnpj=${currentFilters.clienteCNPJ}` : '';
        queryParams += dataInicio ? `&DataPedidoInicio=${currentFilters.dataInicio}` : '';
        queryParams += dataFim ? `&DataPedidoFim=${currentFilters.dataFim}` : '';
        queryParams += currentFilters.statusSeparacao ? `&statusSeparacao=${currentFilters.statusSeparacao}` : '';
